...
/**
 * A class that is the central class for editing a resource database.
 * 
 * This class allows it to add and remove Resources, Type and Groups as well as
 * accessing existing Resources, Types and Groups.
 * 
 * @author Leslie Marxen, Lukas Gross
 */
public class Editor implements Closeable
{
     ...
     * @throws AccessDeniedException      The database file (or a required
     *                                    directory) could not be created.
     * @throws FileAlreadyExistsException The database file already exists.
     * @throws IOException                An I/O error occurred.
     */
    public static Editor newDatabase(File file)
            throws SQLException, AccessDeniedException, FileAlreadyExistsException, IOException
    {
        if (file.exists())
    ...
    private void checkDatabase() throws SQLException
    {
        ...
        checkIfTableHasColumns(Resource.SQL_TABLE, Resource.SQL_COL_ID, Resource.SQL_COL_PATH,
                Resource.SQL_COL_CACHED, Resource.SQL_COL_TYPE);
    ...
    private void checkIfTableExists(String table) throws SQLException
    {
        PreparedStatement stmt = database
                .prepQuery("SELECT * FROM sqlite_master WHERE type = 'table' AND name = ?");
    ...
    private void checkIfTableHasColumns(String table, String... columns) throws SQLException
    {
        ...
        if (!(resultColumns.size() == columns.length) && resultColumns.containsAll(Arrays.asList(columns)))
        {
    ...
    public Resource getResource(int id)
    {
        ...
        ResultSet result = builder.select(Resource.SQL_COL_ID).from(Resource.SQL_TABLE)
                .where(Resource.SQL_COL_ID, id).executeQuery();
    ...
    public Type getType(int id)
    {
        ...
        ResultSet result = builder.select(Type.SQL_COL_ID).from(Type.SQL_TABLE).where(Type.SQL_COL_ID, id)
                .executeQuery();
        try
        {
            /*
    ...
    public Group getGroup(int id)
    {
        ...
        ResultSet result = builder.select(Group.SQL_COL_ID).from(Group.SQL_TABLE).where(Group.SQL_COL_ID, id)
                .executeQuery();
        try
        {
            /*
    ...
    public Resource addResource(String path, String cached, int typeId)
    {
        ...
        int affectedRows = query.insert(Resource.SQL_TABLE).set(Resource.SQL_COL_PATH, path)
                .set(Resource.SQL_COL_CACHED, cached).set(Resource.SQL_COL_TYPE, typeId).executeUpdate();
    ...
    public Type addType(String name, String description)
    {
        ...
        int affectedRows = query.insert(Type.SQL_TABLE).set(Type.SQL_COL_NAME, name)
                .set(Type.SQL_COL_DESCRIPTION, description).executeUpdate();
    ...
    public Group addGroup(String name)
    {
        ...
        int affectedRows = query.insert(Group.SQL_TABLE).set(Group.SQL_COL_NAME, name).executeUpdate();
    ...
    public List<Resource> getResources()
    {
        ...
        ResultSet result = builder.select(Resource.SQL_COL_ID).from(Resource.SQL_TABLE).executeQuery();
    ...
    public List<Resource> getResources(Filter filter)
    {
        ...
        sql.append("SELECT ").append(Resource.SQL_TABLE).append(".").append(Resource.SQL_COL_ID)
                .append(" FROM ").append(Resource.SQL_TABLE);

        // joins
        sql.append(" INNER JOIN ").append(Type.SQL_TABLE).append(" ON ").append(Resource.SQL_TABLE)
                .append(".").append(Resource.SQL_COL_TYPE).append(" = ").append(Type.SQL_TABLE).append(".")
                .append(Type.SQL_COL_ID);

        sql.append(" LEFT JOIN ").append(GROUPS_RESOURCES_SQL_TABLE).append(" ON ").append(Resource.SQL_TABLE)
                .append(".").append(Resource.SQL_COL_ID).append(" = ").append(GROUPS_RESOURCES_SQL_TABLE)
                .append(".").append(GROUPS_RESOURCES_SQL_COL_RESOURCE_ID);

        sql.append(" LEFT JOIN ").append(Group.SQL_TABLE).append(" ON ").append(GROUPS_RESOURCES_SQL_TABLE)
                .append(".").append(GROUPS_RESOURCES_SQL_COL_GROUP_ID).append(" = ").append(Group.SQL_TABLE)
                .append(".").append(Group.SQL_COL_ID);

        sql.append(" WHERE ");
        sql.append(filter.generateSQL());

        // remove duplicate IDs
        sql.append(" GROUP BY ").append(Resource.SQL_TABLE).append(".").append(Resource.SQL_COL_ID);
    ...
    public List<Type> getTypes()
    {
        ...
        ResultSet result = builder.select(Type.SQL_COL_ID).from(Type.SQL_TABLE).executeQuery();
    ...
    public List<Group> getGroups()
    {
        ...
        ResultSet result = builder.select(Group.SQL_COL_ID).from(Group.SQL_TABLE).executeQuery();
    ...
    public boolean removeResource(int resourceId)
    {
        ...
        int affectedRows = builder.delete().from(Resource.SQL_TABLE).where(Resource.SQL_COL_ID, resourceId)
                .executeUpdate();
    ...
    public boolean removeType(int typeId)
    {
        ...
        int affectedRows = builder.delete().from(Type.SQL_TABLE).where(Type.SQL_COL_ID, typeId)
                .executeUpdate();
    ...
    public boolean removeGroup(int groupId)
    {
        ...
        int affectedRows = builder.delete().from(Group.SQL_TABLE).where(Group.SQL_COL_ID, groupId)
                .executeUpdate();
    ...
    // default visibility on purpose; to emulate friends from C++; only Type should
    // call this
